...
const _EVM_NETS = ['hyperevm','ethereum','base','arbitrum','polygon'];
...
function _extractHash(raw) {
  ...
  if (m) return m[1];
  // Bitcoin explorer URLs that use /transactions/btc/<hash> (e.g. blockchain.com)
  const btc = raw.match(/\/transactions\/btc\/([0-9a-fA-F]{64})/i);
  if (btc) return btc[1];
...
function renderPortfolio(tokens) {
  ...
  if (!tokens.length) {
    list.innerHTML = `<div class="empty-state">
      <div class="empty-icon"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/><path d="M2 12h20"/></svg></div>
      <p>${t("empty_trade")}</p>
    </div>`;
    return;
  }
